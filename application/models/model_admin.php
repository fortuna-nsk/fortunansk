...
{	
	...
	{
		if($_SESSION['admin'] == 1){
			$columns = "re_var.id, re_var.user_id, re_var.active, ap_layout, re_var.parent_id, re_company.company_name, re_people.name, re_people.id as people_id, re_people.second_name, re_people.phone, dis, planning, live_point, street, house, orientir, text, topic_id, type_id, price, date_last_edit, sq_all, sq_live, sq_k, floor, floor_count, room_count, coords, deliv_period, prepayment, utility_payment, deposit, inet, furn, tv, washing, refrig, conditioner, ap_view_date, ap_race_date, status, premium, favorit, ap_view_price, in_black_list, residents, DATE_FORMAT(`date_last_edit`,'%d/%m/%Y %H:%i') as `date_last_edit_format`, DATE_FORMAT(`date_added`,'%d/%m/%Y %H:%i') as `date_added_format`";
	...
	{
		if($_SESSION['user'] == 1){
			$active = isset($_GET['active']) && $_GET['active']==0 ? " AND DATE_FORMAT(rent_date_end, '%Y-%m-%d') < DATE_FORMAT(NOW(), '%Y-%m-%d')" : "";
			if($_GET['online']==1){	
				  $data = DB::Select("c.id, c.fortuna_mid, c.company_name, a.rent_date_end, a.sell_date_end, a.pay_parse_date_end", 
				  	"re_company as c, re_access_date_end as a, re_session as s, re_people as p",
				  		 "c.id = a.company_id AND p.company_id = c.id AND s.people_id = p.id AND DATE_FORMAT(DATE_ADD(NOW(), INTERVAL -5 MINUTE), '%Y%m%d%H%i%s') < DATE_FORMAT(s.date_update, '%Y%m%d%H%i%s') AND company_name!='' ORDER BY company_name");
			}else if($_GET['pay_parse']==1){
				$data = DB::Select("c.id, c.fortuna_mid, c.company_name, a.rent_date_end, a.sell_date_end, a.pay_parse_date_end", "re_company as c, re_access_date_end as a, re_people as p", "c.id = a.company_id AND p.company_id = c.id AND a.pay_parse_date_end>NOW() AND company_name!='' GROUP BY company_name ORDER BY company_name");
			}else if($_GET['free_ip']==1){
				$data = DB::Select("c.id, c.fortuna_mid, c.company_name, a.rent_date_end, a.sell_date_end, a.pay_parse_date_end", "re_company as c, re_access_date_end as a, re_people as p, re_addresses as ad", "ad.people_id = p.id AND c.id = a.company_id AND p.company_id = c.id AND ad.ip=1 AND ad.active = 1 AND company_name!='' GROUP BY company_name ORDER BY company_name");
			}else if(isset($_GET['order_access']) && $_GET['order_access']==0){
				$data = DB::Select("c.id, c.fortuna_mid, c.company_name, a.rent_date_end, a.sell_date_end, a.pay_parse_date_end", "re_company as c, re_access_date_end as a, re_people as p", "c.id = a.company_id AND p.company_id = c.id AND c.order_access=0 AND company_name!='' GROUP BY company_name ORDER BY company_name");
			}else if(isset($_GET['duty']) && $_GET['duty']==1){
				$data = DB::Select("c.id, c.fortuna_mid, c.company_name, a.rent_date_end, a.sell_date_end, a.pay_parse_date_end", "re_company as c, re_access_date_end as a, re_people as p", "c.id = a.company_id AND p.company_id = c.id AND c.duty>0 AND company_name!='' GROUP BY company_name ORDER BY company_name");
			}else{
				$data = DB::Select("re_company.id, re_company.fortuna_mid, re_company.company_name, re_access_date_end.rent_date_end, re_access_date_end.sell_date_end, re_access_date_end.pay_parse_date_end", "re_company INNER JOIN re_access_date_end ON re_company.id=re_access_date_end.company_id", "company_name!='' {$active} ORDER BY company_name");
			}
			return $data;
		} else {
			header("Location: http://". $_SERVER['SERVER_NAME']);
	...
	public function find_fio(){
		if($_SESSION['admin'] == 1){
			$data = DB::Select("re_company.id, re_company.fortuna_mid, re_company.company_name, re_access_date_end.rent_date_end, re_access_date_end.sell_date_end", "re_user INNER JOIN re_people ON re_user.people_id = re_people.id	INNER JOIN re_company ON re_people.company_id = re_company.id INNER JOIN re_access_date_end ON re_people.company_id = re_access_date_end.company_id", "surname like '%{$_POST['surname']}%' AND name like '%{$_POST['name']}%' AND second_name like '%{$_POST['second_name']}%' ORDER BY parent");
			return $data;
		}else{
			session_destroy();
		}
	...
	public function messages_list(){
		if(isset($_SESSION['user']) && isset($_POST['to']) && isset($_POST['from'])){
			$fio =  $_POST['fio'];		
			$message = DB::Select("people_id_from, login_from, re_messages.text, DATE_ADD(date_send, INTERVAL -1 hour) as date_send, company_name, phone", 
						...
						"(people_id_from=".$_POST['from']." AND people_id_to=".$_POST['to'].") OR (people_id_to=".$_POST['from']." AND people_id_from=".$_POST['to'].") ORDER BY date_send DESC");
			...
			$num=count($message);
			for($m=0; $m<$num; $m++){
				if($message[$m]['people_id_from'] == $_POST['from'])
				{
					if($_SESSION['admin'] == 1)
					{
						$title = $message[$m]['login_from']." АН «".$message[$m]['company_name']."» ".$fio." т.:".$message[$m]['phone']." ".$message[$m]['date_send'];
					}else{
						$title = $fio." ".$message[$m]['date_send'];
					}
					echo "<div class='item info'><div class='question' data-id='".$_POST['from']."'>".$title."</div>".$message[$m]['text']."</div>";
					
				}else{
				
					echo "<div class='item info' style='text-align: right;'><div class='reply'>".$_SESSION['fio']." ".$message[$m]['date_send']."</div>".$message[$m]['text']."</div>";
				}
			}			
		}else{
			header("Location: http://". $_SERVER['SERVER_NAME']);
	...
	public function message_to_archive(){
		if($_POST['to']==$_SESSION['people_id']){
			DB::Update("re_messages", "archive=1", "people_id_from={$_POST['from']} AND people_id_to={$_POST['to']}");
		}
	}
	
	public function delete_message(){
		if($_POST['to']==$_SESSION['people_id']){
			DB::Delete("re_messages", "people_id_from={$_POST['from']} AND people_id_to={$_POST['to']}");
		}
	}
	...
	public function check_rielter()
	{
		if($_SESSION['user'] && $_POST){
			...
			$check_list = DB::Select("re_check_rielter.id, people_id, search_str, search_result, date_search, check_comment, second_name, name, phone",
					 ...
					 "search_str = '".$_POST['phone']."' ORDER BY `id` DESC  LIMIT 1,500");
			...
			if($_POST['search_type'] == "phone"){
				...
				$data = DB::Select("parent, date_dismiss, second_name, name, phone, phone_addon, phone_archive, date_reg, company_name", 
					"re_people INNER JOIN re_company ON re_people.company_id = re_company.id INNER JOIN re_user ON re_people.id=re_user.people_id", 
					"(phone like '%".$_POST['phone']."%' OR phone_addon like '%".$_POST['phone']."%' OR phone_archive like '%".$_POST['phone']."%') ".$company_id." ORDER BY date_dismiss");
				$count = count($data);
				if($count > 0){
					for($s=0; $s<$count; $s++){
			...
			$query = "SELECT re_check_rielter.id, people_id, company_name, search_str, search_result, DATE_ADD(date_search, INTERVAL -1 hour) as date_search, check_comment, second_name, name, phone,variant FROM re_check_rielter INNER JOIN re_people ON re_check_rielter.people_id = re_people.id INNER JOIN re_company ON re_company.id = re_people.company_id ORDER BY id DESC";
	...
	public function order(){
		if($_SESSION['admin'] == 1){
			$query = "SELECT re_order.id, re_order.active, order_type, card_number, sum, comment_order, DATE_ADD(date_order, INTERVAL -1 hour) as date_order, wallet_num, order_place, pay_date, comment_pay, company_name, company_id FROM re_order INNER JOIN re_company ON re_order.company_id = re_company.id WHERE archive=0 ORDER BY active DESC";
			$res = mysql_query($query);
			$num = mysql_num_rows($res);
			$company=array();
			for($i=0; $i<$num; $i++){
				$data[]= mysql_fetch_assoc($res);
				$company[] = $data[$i]["company_id"];
			}
			if(count($company) > 0){
				$company_str = "p.company_id=".(implode(" OR p.company_id=", $company));
				$query = "SELECT p.company_id, u.login FROM re_user as u INNER JOIN re_people as p ON u.people_id = p.id WHERE u.parent=0 and ({$company_str})";
				$res = mysql_query($query);
				$num = mysql_num_rows($res);
				for($i=0; $i<$num; $i++){
					$p = mysql_fetch_assoc($res);
					$people[$p["company_id"]] = $p["login"];
				}
				$data[0]["people_info"] = $people;
			}
			unset($query, $res, $i, $num);			
			return $data;
		}else{
			header("Location: http://". $_SERVER['SERVER_NAME']);
		}
	...
	public function order_to_archive(){
		if($_SESSION['admin']==1 && $_POST["id"]){
			DB::Update("re_order", "archive=1", "id={$_POST['id']}");
		}
	}
	...
	public function enter_statistics(){
		...
		{
			if($_POST["people_id"]){
				$data = DB::Select("id, people_id, login, ip, browser, DATE_FORMAT(DATE_ADD(date_enter, INTERVAL -1 hour), '%d.%m.%Y %H:%i:%s') as date_enter", "re_enter_statistics", "people_id=".$_POST["people_id"]." ORDER BY id DESC");
				return $data;
			}
		}
	}
	...
	public function delete_employe(){
		if(isset($_POST['user_id']) && $_SESSION['admin']==1){
