		...
		<div class="row">
			<div class="col-xs-12 deployed">
				<div class="col-xs-2 deployed">
					<label class="signature">Дата аренды</label>
					<input type="text" onFocus="$(this).mask('9999-99-99')" name="ac-rent_date_end" class="form-control" placeholder="дата" value="<?echo $data[0]['rent_date_end'];?>">
				</div>
				<div class="col-xs-1 deployed">
					<label class="signature">Прем.А</label>
					<input type="text" name="co-rent_premium" class="form-control" placeholder="периум" value="<?echo $data[0]['rent_premium'];?>">
				</div>
				<div class="col-xs-1 deployed">
					<label class="signature">Абонентка</label>
					<input type="text" name="co-subscription" class="form-control" placeholder="абонентка" value="<?echo $data[0]['subscription'];?>">
				</div>				
				<div class="col-xs-2 deployed">
				...
				<div class="col-xs-2 deployed">
					<label class="signature">Дата частники 2</label>
					<input type="text" onFocus="$(this).mask('9999-99-99')" name="ac-pay_parse_date_end" class="form-control" placeholder="дата" value="<?echo $data[0]['pay_parse_date_end'];?>">
				</div>
				<div class="col-xs-1 deployed">
					<label class="signature">Баланс</label>
					<input type="text" name="co-balance" class="form-control" placeholder="остаток" value="<?echo $data[0]['balance'];?>">
				</div>
				<div class="col-xs-1 deployed">
					<label class="signature">Долг</label>
					<input type="text" name="co-duty" class="form-control" placeholder="остаток" value="<?echo $data[0]['duty'];?>">
				</div>	
		...
		<form class="employee col-xs-12 director <?/*if($data[$e]['parent']==0) echo 'director';*/?>" data-id="<?echo $data[$e]['user_id'];?>">
			<div class="row fio">
				...
				<div class="col-xs-3">
					<div class="checkbox" style="display: inline-block;">
						<label>
							<input type="checkbox" name="rent_on" value="1" <?if($data[$e]["group_topic_id"]!=2) echo "checked";?>>
							Аренда
						</label>
					</div>
					<div class="checkbox" style="display: inline-block;">
						<label>
							<input type="checkbox" name="sell_on" value="2" <?if($data[$e]["group_topic_id"]!=1) echo "checked";?>>
							Продажа
						</label>
					</div>
				</div>
				...
				for($p=0; $p<count($phones_addon); $p++){
					if(ereg('8', $phones_addon[$p])){
						echo "<div class='col-xs-2 deployed'>
								<label class='signature'>Телефон для созвонов</label>
			...
			<div class="row fio">
				...
				for($i=0; $i<count($address_rent); $i++){
					echo "<div class='col-xs-2 deployed'><label class='signature'>IP аренды</label><input type='text' name='ad-rent_ip-".$address_rent[$i]['id']."' class='form-control' placeholder='ip' value='". $address_rent[$i]['ip']."' required disabled></div>";
				}?>
				<div class="col-xs-3 deployed">
					<div class="input-group interval xl ip" data-id="new">
						<input type="text" class="form-control" placeholder="ip" id="ip">
						<button type="button" onClick="add_ip('<?echo $data[$e]['user_id'];?>', 'rent')" data-user="<?echo $data[$e]['user_id'];?>" class="form-control btn btn-success right">добавить</button>
					</div>
				</div>
			</div>
			<!--<div class="row">
			...
			<div class="col-xs-2 deployed fio">
				...
				<button type="button" class="form-control btn btn-info" target="_blank" data-toggle="modal" data-target="#clean-modal-win" onClick="EnterStatistics(<?echo "'".$data[$e]["people_id"]."', '".$fio."'";?>)">Статистика входов</button>
			</div>
			<div class="col-xs-2 deployed fio">
