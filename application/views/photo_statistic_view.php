	...
	<?if(!isset($_POST["login"]) && !isset($_POST["street"])){?>
			<thead id="main_table">
				<tr>
					...
					<th>Логин</th>
					<th>Ip</th>
					<th>Просмотров за последнии 3 дня</th>
				</tr>
			</thead>
			<tbody>
				<?
		...
		<?}else{?>
			...
			if(isset($_POST["login"])){	
				$res = DB::Select("p.login, p.ip, DATE_FORMAT(p.date, '%H:%i:%s %d.%m.%Y') as date, v.street, v.house, v.id as var_id, v.parent_id as var_parent_id", "re_photo_statistic as p, re_var as v", "p.var_id = v.id AND p.login='{$_POST['login']}' ORDER BY p.date DESC");
