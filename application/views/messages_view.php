...
<div class="col-xs-9">
	...
	<table id="application" class="table table-striped">
		...
		<tbody>
			...
			?>
			<tr class='message' id='<?=$data[$i]['id'];?>'>
				<td><?=($i+1);?></td>
				<td onClick="Messages(<?=$data[$i]['people_id_from'];?>, <?=$_SESSION['people_id'];?>, '<?=$fio;?>')"  target='_blank' data-toggle='modal' data-target='#messages-modal-win'><?=$fio;?> <?=($admin ? " (".$data[$i]['login_from'].")" : '');?></td>
				...
				<td><span class="dropdown">
						...
						<ul class="dropdown-menu" aria-labelledby="messagesMenu" style='margin-left: -115px;'>
							<li><a href="javascript:void(0)" onClick="ToArchive('message', '<?echo $data[$i]['people_id_from']."|".$_SESSION['people_id'];?>')">В архив</a></li>
							<li><a href="javascript:void(0)" onClick="Delete('message', '<?echo $data[$i]['people_id_from']."|".$_SESSION['people_id'];?>')">Удалить</a></li>
						</ul>
					</span>
				</td>
			</tr>
