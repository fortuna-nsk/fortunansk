...
<div style="width: 215px;display: inline-block;float: left;">
	<ul class="admin-menu">
		...
		<a href="?task=admin&action=order"><li class='<?if($_GET['action']=='order')echo "active";?>'>Оплата (<?echo Get_functions::Get_new_order_count();?>)</li></a>
		<li class='<?if($_GET['action']=='find_an' || $_GET['action']=='find_employee')echo "active";?>'>
			...
			<form id='phone' action='?task=admin&action=find_phone' method='POST'>
				<input style='width:150px; display:inline-block' class='form-control' type='text' data-id='phone'  form='phone' name='phone' placeholder='телефон' value='<?if(isset($_POST['phone']))echo $_POST['phone'];?>'>
