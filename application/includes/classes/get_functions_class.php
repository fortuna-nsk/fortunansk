...
{		
	...
	static function Get_black_list_show_var($people_id) {
		$query = "SELECT show_var FROM re_black_list WHERE 
				`owner_people_id` = '{$_SESSION['people_id']}' AND `people_id` ={$people_id}";
		return  mysql_fetch_assoc(mysql_query($query))['show_var'];		
	}
	...
	static function Get_premium_balance() {
		if(isset($_SESSION['company_id'])){
			...
			$condition = "company_id=".$_SESSION['company_id']." AND (topic_id=".$_GET['topic_id']." OR topic_id=". ($_GET['topic_id'] += 2) .")";
			$sum = DB::Select("SUM(premium)", $table, $condition);			
			$premium_count = Get_functions::Get_premium_count();
			$result = $_GET['topic_id'] == 5 || $_GET['topic_id'] == 3 ? ($premium_count['rent_premium'] - $sum[0]['SUM(premium)']) : ($premium_count['sell_premium'] - $sum[0]['SUM(premium)']);
			return $result;
		}
	}
	...
	static function Get_suspicion_text($text){
		...
		if($duo1 || $duo2 || sizeof($out[0]) > 3 || $suspWord) return 1;
			return 0 ;
	}
		...
		if($num>0){
			for($i=0; $i<$num; $i++){
				list($surname, $name, $second_name) =  explode(" ", $group_array[$i]['group_owner']);
					$groupListAdd = DB::Select("`user_id`", "`re_user` INNER JOIN `re_people` ON re_people.id=re_user.people_id", "`surname`='".$surname."' AND `name`='".$name."' AND `second_name`='".$second_name."'");
					if(isset($groupListAdd[0]['user_id'])) $group_list.= $groupListAdd[0]['user_id'].",";
			}
		}
	...
	static function Get_city_list(){	
		if(($_SESSION['search_user_id'] == "site" && $_GET['action']!="pay_parse") || $_GET["action"]=="mytype"){
			$table = "re_var";
	...
	{
		if($what == 'all'){
			$count = DB::Select("count(*) as c", "re_company as c,  re_access_date_end as a", "c.id=a.company_id GROUP BY(DATE_FORMAT(a.rent_date_end, '%Y-%m-%d') < DATE_FORMAT(NOW(), '%Y-%m-%d'))");
	...
	static function Get_chat_mess_list(){
		if(isset($_SESSION["user"])){
			$mess = DB::Select("text, nickname, user_id, DATE_FORMAT(date,'%H:%i %d.%m') as date", "re_chat", "DATE_ADD(date,INTERVAL +30 day) > NOW()");
			$count = count($mess);
			for($m=0; $m<$count; $m++){
				echo "<div class='mess'>
						<div><a href='javascript:void(0)' data-user-id='{$mess[$m]['user_id']}' data-toggle='tooltip' data-placement='left' title='' class='nickname'>{$mess[$m]['nickname']}</a><span class='messTime'>{$mess[$m]['date']}</span></div>
