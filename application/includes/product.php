...
// if($_SESSION['post']['view_type'] == "map"){
	// echo "<div id='map' style='width: 100%; height: 550px; display:none'></div>";
// }
	...
	<div class="col-xs-12 product" data-coords="<?echo $data[$j]['coords'];?>" id="msg<?php echo $j; ?>" data-id='<?echo $data[$j]['id'];?>' data-addr="НСО, <?echo $data[$j]['live_point'].", ".$data[$j]['street']." д.".$data[$j]['house'];?>">
		...
		<div class="col-xs-7 product-description">
			<span id="product-view">
				...
				if(!$ngs){
					...
					if($favorit){
						echo "<a href='javascript:void(0)' onClick='addToFavorites(\"".$data[$j]['user_id']."\", ".$data[$j]['id'].")'><img src='images/icon/favorites.png' style='cursor:pointer;height: 20px;margin-left: -25px;vertical-align: top;margin-right: 5px;'></a>";
					}else{
						echo "<a href='javascript:void(0)' onClick='removeFromFavorites(\"".$data[$j]['user_id']."\", ".$data[$j]['id'].")'><img src='images/icon/favorites_all.png' style='cursor:pointer;height: 20px;margin-left: -25px;vertical-align: top;margin-right: 5px;'></a>";
					}
				}
		...
		<div class="col-xs-12">
			<? if($_GET['active'] == '1' && ($my_var || $_SESSION['parent']==0)){
				...
				<span id='control'><a href='javascript:void(0)' onclick='VarExtend(".$data[$j]['id'].")'>Продлить вариант</a></span>";
