	...
	<div class="col-xs-12 product" data-coords="<?= $data[$j]['coords'];?>" id="msg<?= $j; ?>" data-id='<?= $data[$j]['id'];?>' data-addr="НСО, <?= $data[$j]['live_point'].", ".$data[$j]['street']." д.".$data[$j]['house'];?>">
		...
		<div data-id="<?echo $data[$j]['status'];?>">
			<span id="product-view">
				...
				if($favorit){
					echo "<a href='javascript:void(0)' onClick='addToFavorites(\"".$data[$j]['user_id']."\", ".$data[$j]['id'].")'><img src='images/icon/favorites.png' style='cursor:pointer;height: 20px;vertical-align: top;margin-right: 5px;'></a>";
				}else{
					echo "<a href='javascript:void(0)' onClick='removeFromFavorites(\"".$data[$j]['user_id']."\", ".$data[$j]['id'].")'><img src='images/icon/favorites_all.png' style='cursor:pointer;height: 20px;vertical-align: top;margin-right: 5px;'></a>";
				}
