...
function changePage(page) {		
	...
	// jQuery.ajax({		
		// type: 'POST',
		// url: '?task=main&action=save_page', 
		// data: 'PAGE=' + page, 		
		// success: function(){
			// $(".products-list .product").hide();
			// $(".pagination_row").hide();	
			// $(".products-list").append("<img src='/images/ajax-loader.gif' alt='Загрузка' style='  width: 200px; margin-left: 40%;'>");
		// },
		// complete: function() { 										
...
onReady(function() {
    ...
    var divs = document.body.getElementsByTagName('body')
	var page = 0
	var number = document.getElementById('msg-box').getElementsByTagName('div').length
	var limit = document.getElementById('limit').value
	var summ = parseInt(page*limit) + parseInt(limit)
	
	
	//console.log(limit)
	
	for(var n=0; n<number; n++) {
		if ((n >= page*limit) && (n < summ)) {
					...
					success: function(html){
						if(userList){
							$(button).parent().parent().before(
								"<div class='col-xs-2 deployed'><label class='signature'>Телефон для созвонов</label><input type='text' onFocus='$(this).mask(\"8 (999) 999-9999\")' name='pe-phone_addon"+($(".employee[data-id=1] .row.phone").children().length-1)+"' class='form-control' placeholder='Телефоны для созвонов' value='"+phone+"' required disabled></div>"
							);
						}else{
...
function add_ip(id, rentOrSell){
	var button = $(".ip button[data-user="+id+"]"),
		ipInput = $(button).parent().find("input"),
		ip = $(ipInput).val();
		if(ip.match(/[\d+\.]+/) != null){
			alertify.confirm("Добавить новый ip-адрес "+ip+" ?", function(result){
				if(result){		
					$.ajax({
						...
						data: 'ip=' + ip + "&user="+id + "&rentOrSell="+rentOrSell, 
						success: function(ipId){
							$(button).parent().parent().before(
							"<div class='col-xs-2 deployed'><input type='text' name='ad-"+rentOrSell+"_ip-"+ipId+"' class='form-control' placeholder='ip' value='"+ip+"' required='' disabled=''></div>"
							);
							$(ipInput).val("");
						}
					})
...
$(document).ready(function() {
	...
	$(document).on("click", ".admin-mes .close", function(){
		$.post("?task=chat&action=read_mes");
	})
	...
	// $(document).on("click", "[name=live_point],[name=dis],[name=street],[name=house]", function(){
		// if($("#map").is(":visible")){
			// $("#map").hide();
		// }
	// })
	...
	$(document).on("click", "[data-name=check-var]", function(){
		...
		$.get("?task=main&action=check_var", "var_id="+id+"&table="+table, function(html){
			...
			if($(html).find(".products-list .product").length > 0){
				...
				// $(".close").click();
				// alertify.alert("Похожих вариантов от частников нет.");
	...
	$(document).on("click",".fancybox-thumbs img", function (e){
		...
		if($(link).length == 1 && $(link).attr("href").match(/main.jpg||zenit1.png/) != null){
			...
			jQuery.ajax({
				...
				data: "varId="+$(obj).data("fancybox-group")+"&url="+$(obj).attr("href"),
				success: function(html){
					if(html=='over'){
						...
						// }
						// if($(link).attr("href").match(/main.jpg/) != null){
							// var href = $($('[data-fancybox-group='+dataFGroup+'] img')[1]).attr('src');
							// $($('[data-fancybox-group='+dataFGroup+']')[0]).attr('href', href);
							// $($('[data-fancybox-group='+dataFGroup+'] img')[0]).attr('src', href);
							// $($('[data-fancybox-group='+dataFGroup+']')[1]).remove();
						// }
						...
						$(".progress").remove();
						//$($('[data-fancybox-group='+dataFGroup+'] img')[0]).click();
					}
				}
			});	
			e.preventDefault();
...
function VarExtend(id)
{
	...
	if (id == "checked"){
		id = "";
		$(".product :checked").each(function(){
			// dateLastEditArr = $(this).parent().find("[data-id=last-edit]").text().split(" ");
			// dateArr = dateLastEditArr[0].split("/");
			// timeArr = dateLastEditArr[1].split(":");
			// dateLastEdit = new Date(date.getFullYear(), parseInt(dateArr[1])-1, dateArr[0], parseInt(timeArr[0])+1, timeArr[1]);
			// if(date >= dateLastEdit){
				// id += $(this).data("id")+",";
			// }
			id += $(this).data("id")+",";
		});
	...
	// else{
		// dateLastEditArr = $("[type=checkbox][data-id="+id+"]").parent().find("[data-id=last-edit]").text().split(" ");
		// dateArr = dateLastEditArr[0].split("/");
		// timeArr = dateLastEditArr[1].split(":");
		// dateLastEdit = new Date(date.getFullYear(), parseInt(dateArr[1])-1, dateArr[0], parseInt(timeArr[0])+1, timeArr[1]);
		// if(date < dateLastEdit){
	...
	// }
	if(id != ""){
		alertify.confirm('Потверждаю, что на момент продления вариант актуален. Продлевать варианты можно 1 раз в час. Будте внимательны!', function(result){
			if (result) {
				jQuery.ajax({
					type: 'POST',
					...
					success: function() {
						...
						// var ids_array = id.toString().split(','),
							// count = ids_array.length == 1 ? 2 : ids_array.length;
						// for(i=0; i<count - 1; i++){
							// var productDiv = $(".product").has(":checkbox[data-id="+ids_array[i]+"]");
							// $(productDiv).find(".product-description").prepend("<h3 class='right' style='color:rgb(92, 184, 92);'>Продлен</h3>");
			...
			success: function(html){
				if(html == "1"){
					...
					if(QueryString("action") == "find_employee"){
						$($("#"+formId).find("td")[3]).text($("[data-id="+formId+"] [name=ac-rent_date_end]").val());
			...
			$.ajax({
				...
				success: function(html){
					...
					if(!$("#anonymous").is(":checked") && $(".product[data-id="+id+"] [data-id=review]").length == 0){
						$(".product[data-id="+id+"] div").last().append(" | <a href='javascript:void(0)' style='color:#E81010' data-id='review' target='_blank' data-toggle='modal' data-target='#clean-modal-win'>Есть отзывы</a>")
					}
...
function Check_comment_set(io, date, searchStr, peopleId){
			...
			success:function(html){
				if(html = 1){
					if($("div#check_list").length == 0){
						...
						$("div#check_list tbody").append("<tr><td>"+number+"</td><td>"+io+"</td><td>"+searchStr+"</td><td>"+date+"</td><td>"+text+"</td></tr>");
					}
					$("div.info").slideUp();
...
function Open_send_email_form(obj){
	...
	if(!$(obj).next().hasClass('hidden')){
		var product = $('.product').has($(obj)),
			title = $($(product).find('td').last().find('span')[1]).text(),
...
function loginShow(id) {
	var enter_form = $("form#enter");
	if (id == 1){
		$(enter_form).slideDown();
	};
	//$(enter_form).css("margin-left", $("div.container").width() - $(enter_form).width() - 35);
}
