<script type="text/javascript">
	setTimeout(function(){
		window.location = "updateCoord.php";
	}, 1000);
</script>
<?
	...
	function translate_var_to_new($data, $db1, $db2)
	{
		...
		if($data['conti']=='2'){
			...
			$planning =  'смежная';			
		}else if($data['conti']=='6'){
			$ap_layout =  'улучшеная планировка';
		}
		...
		if(ereg('syo', $data['dop_price'])){
			$utility_payment = "дополнительно только за свет";				
		}else if(ereg('vyo', $data['dop_price'])){
			$utility_payment = "дополнительно только за воду";
		}else if(ereg('sy', $data['dop_price'])){
		...
		if(isset($data['zaezd']) && isset($data['smotr'])){
			$date_smotr = date("Y")."-".substr($data['smotr'], 4, 2)."-".substr($data['smotr'], 6, 2);
			$date_zaezd = date("Y")."-".substr($data['zaezd'], 4, 2)."-".substr($data['zaezd'], 6, 2);
		}else{
			...
			if(intval($people_id) == 0){
				if($rieltor['comm'] == "директор"){
					...
					if($parent_id == ""){
						...
						$director = mysql_fetch_assoc($res);
					...
					unset($res);
					$res = mysql_query("SELECT login FROM re_user GROUP BY login", $db1);
					while($login = mysql_fetch_assoc($res)){
						$logins[] = $login["login"];
					}
					...
					$res = mysql_query("SELECT user_id FROM re_user WHERE people_id='".$people_id."' AND active=1 AND archive=0 AND login = '".$login."'", $db1);
					return mysql_fetch_assoc($res)['user_id'];
				}
			}else{
				$res = mysql_query("SELECT user_id FROM re_user WHERE people_id='".$people_id."' AND active=1 AND archive=0", $db1);
				return mysql_fetch_assoc($res)['user_id'];
			}
		}
	}
	...
	// $photos_res = mysql_query("SELECT var_id, people_id from re_photos WHERE water_mark = 0 GROUP BY var_id ORDER BY date_added DESC", $db1);
	// while($photo = mysql_fetch_assoc($photos_res)){
		// $photo_dir = $_SERVER['DOCUMENT_ROOT'].'/images/'.$photo['people_id'].'/'.$photo['var_id'];
		// //add_water_mark($photo['people_id'], $photo['var_id'], $photo_dir, $db1);
	// }
	
	$res = mysql_query("select t.*, l.name, l.phone from text_smart as t, list as l where l.mid = t.mid AND t.object != 20 and t.object != 13 and t.object != 14 and t.date_opt >= DATE_FORMAT(DATE_ADD(NOW(), INTERVAL  -72 MINUTE),'%Y%m%d%H%i') ORDER BY t.date_opt", $db2);
